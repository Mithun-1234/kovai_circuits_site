...
const swiffyslider = {
	...
	autoPlay(e, t, i) {
		...
		const childDiv = document.querySelector('.pause');
		if (childDiv) {
...
const siteToggle = document.querySelector('.site-toggle i');
if (siteToggle) {
	siteToggle.onclick = function (e) {
...
if (_siteHeader) setTimeout(() => _siteHeader.classList.add('show'), 100);

const _heroSlider = document.querySelector('.hero-slider');
if (_heroSlider) setTimeout(() => _heroSlider.classList.add('show'), 100);

const _heroAnchor = document.querySelector('.hero-slider .hero-content .anchor-div');
if (_heroAnchor) setTimeout(() => _heroAnchor.classList.add('show'), 2000);
...
(function () {
	...
	const desc = document.getElementById('srvModalDesc');
	const icon = document.getElementById('srvModalIcon');
	...
	document.querySelectorAll('.kc-srv-card').forEach((card) => {
		card.addEventListener('click', () => openModal(card));
	});
	modal.querySelector('.kc-modal-close').addEventListener('click', closeModal);
...
// ============ TESTIMONIAL SWIPER ============
(function () {
	const swiperEl = document.querySelector('.kc-testimonial-swiper');
	...
	new Swiper('.kc-testimonial-swiper', {
		...
		pagination: {
			el: '.kc-testi-pagination',
			clickable: true,
		},
		navigation: {
			nextEl: '.kc-testi-next',
			prevEl: '.kc-testi-prev',
		},
		breakpoints: {
